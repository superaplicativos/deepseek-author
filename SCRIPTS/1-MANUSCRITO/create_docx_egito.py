...
do livro "Turma da Aventura: Os Segredos das Pirâmides"
"""
...
def add_cover_page(doc):
    ...
    subtitle_run = subtitle.add_run("Os Segredos das Pirâmides")
    ...
    series_run = series.add_run("Livro 3 da Série Lugares Incríveis")
    ...
    info_run = info.add_run("Uma Aventura no Egito Antigo")
...
def add_credits_page(doc):
    ...
    credits.add_run("Livro 3: Os Segredos das Pirâmides\n\n")
    ...
    credits.add_run("Livros anteriores: Atlântida (Livro 1), Os Ecos de Constantinopla (Livro 2)\n")
    credits.add_run("Próxima aventura: Em breve!")
...
def add_back_cover(doc):
    ...
    synopsis.add_run(
        "Três semanas após desvendar os mistérios de Constantinopla, a Turma da Aventura "
        "recebe um novo chamado: cristais atlantes estão pulsando sob as Pirâmides de Gizé. "
        "Viajando para o Egito de 2560 a.C., durante a construção da Grande Pirâmide de Khufu, "
        "Will, Mia, Leo, Sophie, Max, Jimmy e Íris encontram Nadia, uma guardiã descendente "
        "de antigas linhagens que protege segredos milenares.\n\n"
    )

    synopsis.add_run(
        "Nas profundezas sob as pirâmides, descobrem câmaras secretas onde tecnologia atlante "
        "se entrelaça com a sabedoria egípcia. Mas algo desperta nas sombras - um guardião antigo "
        "que testa não apenas sua coragem, mas sua compreensão do equilíbrio entre conhecimento "
        "e responsabilidade. Entre hieróglifos misteriosos e passagens ocultas, a turma precisa "
        "decifrar enigmas que conectam Atlântida ao Egito Antigo.\n\n"
    )

    synopsis_run = synopsis.add_run(
        "Uma jornada épica através das areias do tempo, onde cada descoberta revela que "
        "a história da humanidade é mais interconectada - e mais mágica - do que jamais imaginaram."
    )
    ...
    next_run = next_adv.add_run("CONTINUE A AVENTURA")
    ...
    next_title.add_run("Série Lugares Incríveis").font.italic = True

    next_desc = doc.add_paragraph()
    next_desc.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
    next_desc.add_run("Novos lugares extraordinários aguardam a Turma da Aventura!")
...
def main():
    ...
    with open('D:\\TRAE-PROJETOS\\livro1\\BIZANTINO\\LIVROS\\SERIE-LUGARES-INCRIVEIS\\LIVRO-03-PIRAMIDES\\PT\\manuscrito_egito.txt', 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    # Salvar documento
    output_path = 'D:\\TRAE-PROJETOS\\livro1\\BIZANTINO\\LIVROS\\SERIE-LUGARES-INCRIVEIS\\LIVRO-03-PIRAMIDES\\PT\\Turma_da_Aventura_Os_Segredos_das_Piramides.docx'
